...
Work carefully through each of the sections below, confirming that all your outputs match ours. When you feel completely comfortable with the content, we recommend moving on to Section 3 of R for Data Science: Data Visualisation (https://r4ds.had.co.nz/data-visualisation.html) which covers advanced plotting with the ggplot library.
...
include_graphics("docs/images/donations_puppy.png")
```
...
R also gives you some information about how it understands the contents of the file it has imported. In the console you will see the number of rows and columns in the imported data. You will also see the words **Column specification** followed by a list of the imported columns. 

Each column header is associated with a value **dbl**, or **chr**. These indicate what kind of data R identified in the input file. Columns that are **dbl** are numbers; columns that are **chr** are strings. The same information is shown where RStudio prints the imported data. Beneath each column header you will see **dbl** for numbers and **chr** for strings. 
...
```
### Boxplots
We can see that the mean amount is higher for collectors who had puppies than for those who did not ($197.94 vs. $105.84). We can illustrate this difference graphically using a boxplot. 
...
include_graphics("docs/images/markdown_replace_code.png")
```
...
Ensure you have the `tidyverse` package installed. We will be using this package in our next module. Refer to Appendix 2 below for detailed package installation instructions..
 
To learn how to make more complex graphs, explore Section 3 - Data Visualisation in the online text "R for Data Science" (https://r4ds.had.co.nz/data-visualisation.html). This material covers **ggplot**, the industry-standard graphics package for R. Be prepared: The ggplot syntax is probably different from anything you have used before, and it can take a little getting used to. But be patient, because with ggplot you can produce publication-quality plots and figures that will definitely enhance your own research project reports. As always, if you run into trouble, let us know.


# Appendix

## 1. Manual Data Entry in R
...
When R is installed, it comes with a large set of commands, data structures, and functions which are together known as **base R**. Over the years, researchers and programmers have written thousands of additional functions and modules (called **libraries**) that you can use in your own R code. This **third-party code** is checked by moderators and, if it passes inspection, loaded onto an official R server in a file structure called a **package**. To use packages, you must first download them to your computer. They are placed in the same folders where base R was originally installed. 
...
# Download the readr code to your machine
...
The tidyverse is a collection of packages (metapackage) that offer a structured syntax for performing data preparation and basic analysis in R. Running `install.packages("tidyverse")` installs all the individual packages that make up the tidyverse core.

The core tidyverse packages are `ggplot2` (plotting), `dplyr` (data manipulation), `tidyr` (data tidying), `readr` (data importing), `purrr` (functional programming), `tibble` (a special type of data frame), `stringr` (common tasks for string manipulations), and  `forcats` (dealing with factors).
